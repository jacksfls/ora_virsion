...
EOL
	...
	su oracle -c "NLS_LANG=.$CHARACTER_SET $ORACLE_HOME/bin/impdp IMPDP/IMPDP directory=IMPDP dumpfile=$DUMP_FILE $IMPDP_OPTIONS nologfile=y"
        su oracle -c "NLS_LANG=.$CHARACTER_SET $ORACLE_HOME/bin/sqlplus -S / as sysdba @/tmp/game.sql"
	#Disable IMPDP user
...
case "$1" in
	'')
		...
		if [ "$(ls -A /u01/app/oracle/oradata/XE 2> /dev/null)" ]; then
